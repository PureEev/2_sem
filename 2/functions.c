...
void SystemInitialise(char* fileName, HDC hdc, HWND hWnd) {
	...
	while (fgets(str, 256, file)) {
		if (strcmp(str, "\tButtonBegin\n") == 0) {
			...
			sscanf(str, "\t\tPosition=(%d,%d)\n", &newButton->position.x, &newButton->position.y);
			sscanf(str, "\t\tColor=(%d,%d,%d)\n", &newButton->color[0], &newButton->color[1], &newButton->color[2]);
			sscanf(str, "\t\HiglightColor=(%d,%d,%d)\n", &newButton->HighlightColor[0], &newButton->HighlightColor[1], &newButton->HighlightColor[2]);
			sscanf(str, "\t\tName=%s\n", newButton->name);
		}
		else if (isButton == 0 && newWindow != NULL) {
			sscanf(str, "\t\tPosition=(%d,%d)\n", &newWindow->position.x, &newWindow->position.y); 
			sscanf(str, "\t\tColor=(%d,%d,%d)\n", &newWindow->color[0], &newWindow->color[1], &newWindow->color[2]);
		}
	}
